-- Keep the daily duplicate-session limit for therapists, but allow admins to
-- mark additional sessions when correcting or supervising same-day therapy work.
...
begin
  ...
  if actor_role = 'therapist' and sessions_today >= 2 then
...
comment on function public.mark_session_atomic(uuid, uuid)
  is 'Append-only package session marker for therapists/admins. Therapists are limited to two sessions per package per day; admins can override same-day marking.';
